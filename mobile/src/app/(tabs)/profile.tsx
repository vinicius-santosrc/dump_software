...
export default function ProfileTab() {
        ...
        const getUserData = async () => {
            try {
                ...
                if (currentUser?.username) {
                    try {
                        profileUser = await profileService.getUserByUsername(currentUser.username);
                    } catch {
                ...
                await profileService.setBackgroundFromImage(
                    profileUser?.profilePictureUrl || profileUser?.thumbnail,
    ...
    const filteredPosts = useMemo(() => {
        switch (activeTab) {
            case "media":
                return userPosts.filter((post: Post | any) => (post?.media?.length ?? 0) > 0);
            case "likes":
                return userPosts.filter((post: Post | any) => (post?.likes?.length ?? 0) > 0);
            default:
                return userPosts;
        }
    }, [activeTab, userPosts]);
...
function FlatProfileGrid({
    ...
    renderPost: ({ item }: { item: Post | any }) => React.ReactElement;
    headerOffset: number;
}) {
    return (
        <FlatList
