...
export class ExploreFacadeService {
    ...
    getExploreState(): Observable<ExploreState> {
        return this.topicService.getTrending(10).pipe(
            catchError(error => {
            ...
            switchMap(({ rawTopics, normalizedTopics }) => {
                ...
                const sectionRequests = topicsToLoad.map(topic =>
                    this.searchService.searchOnce(topic.label).pipe(
                        map(response => this.createSectionFromSearch(topic, response.posts ?? [])),
                        catchError(error => {
                            console.error(`[EXPLORE] Erro ao buscar posts do tópico ${topic.label}`, error);
                            return of(this.createEmptySection(topic));
                        })
                    )
                );
    ...
    sharePost(post: ExplorePostCard): void {
        post.shares += 1;
    }
